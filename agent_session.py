...
class Session:
    ...
    def __init__(self, workspace=None, share=CONTEXT_SHARE):
        ...
        # Two different questions, and running them together is what made the
        # readout unreadable. `tokens_in` is how big the request in flight is
        # -- how full the context window is, which is the figure a user can
        # act on. `tokens_sent` is every request added up, which is the spend.
        # It used to be only the second, under the first one's name, so a
        # single question answered in three steps read as a context that had
        # tripled when nothing had grown at all: the same prompt had been sent
        # three times.
        self.tokens_in = 0
        self.tokens_sent = 0
        self._tokens_out = 0
    ...
    def record_request(self, messages):
        """Measure the request about to go, and add it to the running spend.
        ...
        `tokens_in` is set rather than added to. Every step of a turn re-sends
        the whole request -- the system prompt, the carried turns, the task --
        because the API is stateless and there is nowhere else for them to
        live, so adding each one to the last measured the same prompt over and
