...
- TMT runs commands through exactly one action, `bash`, and it runs them under a
  ...
  set out in [Running commands](#running-commands-the-bash-tool), including the part
  it cannot do.
...
hand](#putting-a-key-in-by-hand) below. Set `OPENROUTER_API_KEY` to skip the question
...
these live in is the [installation directory](#the-two-directories) — `~/.tmtcode`
...
are watching. See [Background agents](#background-agents).
...
[Running commands](#running-commands-the-bash-tool).
...
| `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY` | from `.tmt_providers.json`. See [Putting a key in by hand](#putting-a-key-in-by-hand) |
