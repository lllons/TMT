...
def test_typing_a_slash_offers_the_commands_and_narrows_as_you_type():
    assert [name for name, _ in agent_commands.completions("/")] == [
        ...
        # Last, and together, because they are the only two here that can
        # change anything -- everything above reads.
        "/undo", "/checkpoints"]
    ...
    assert [name for name, _ in agent_commands.completions("/c")] == [
        "/context", "/config", "/clear", "/checkpoints"]
    # `/c` therefore no longer completes to anything: four candidates agree
    # only on the "/c" already typed, and a completion that guessed would be
    # worse than none.
    assert agent_commands.completed("/c") == ""
    assert [name for name, _ in agent_commands.completions("/ch")] == ["/checkpoints"]
    assert [name for name, _ in agent_commands.completions("/u")] == ["/undo"]
...
def test_the_caret_counts_the_rows_offered_under_the_line():
    ...
    # Counted rather than written out: `/c` is context, config, clear and
    # checkpoints today, and the next command beginning with a c would make a
    # literal here fail a test that is about the CARET rather than about how
    # many commands exist.
    for typed, offered in (("/mo", 1), ("/c", len(agent_commands.complete("/c"))),
